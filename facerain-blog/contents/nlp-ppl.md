...
모델 간 성능 비교를 위해서는 **평가 지표**라는 것이 필요합니다. 이번 시간에 알아보는 PPL은 이러한 평가 지표 중 하나입니다.
...
하지만 여기서 주의할 점은 PPL이 낮다고 무조건 좋은 언어 모델은 아니라는 것입니다. PPL이 낮아도 사람 입장에서는 어색한 문장일수도 있습니다. 따라서 PPL을 측정할 때는 많은 양의 그리고 도메인에 알맞는 테스트 문장들을 이용했는지 확인해야합니다.
...
위 식에서 $P(w_{1}, w_{2}, w_{3}, ... , w_{N}) 은 언어 모델에서 주어진 문장이 나타날 확률을 의미합니다. 이 확률 값에 기하 평균을 구하고, 역수를 취한 것이 바로 PPL입니다. 
만약 언어 모델이 주어진 문장을 생성할 확률이 높다면 PPL이 낮아지게 됩니다. 여기서 미리 테스트 문장을 선정하여 모델의 PPL을 측정하게 됩니다.     

> 문장을 생성할 확률에 기하평균을 취해주는 이유는 무엇인가요?
> 문장을 생성할 확률인 $P(w_{1}, w_{2}, w_{3}, ... , w_{N})은 문장이 길어질수록 매우 작은 확률값을 가지게 됩니다. 이를 방지하기 위해 곱의 평균을 의미하는 기하 평균을 취해주게 되는 것입니다.  

위 식을 **n-gram** 에 대해서도 적용할 수 있습니다.
위 식을 **연쇄 법칙** 을 이용하면 아래와 같이 나타낼 수 있습니다.
$$
...
지금까지 PPL을 구하는 방법에 대해 알아보았습니다.  
**PPL은 테스트 문장들만 있으면 간단하고 빠르게 모델을 평가할 수 있는것이 특징입니다.** 하지만 앞서 설명한 것처럼 PPL이 낮다고 반드시 꼭 좋은 언어 모델은 아닙니다.
...
$$
이는 매 time-step마다 6가지의 경우의 수가 있음이라고 해석할 수 있습니다.
